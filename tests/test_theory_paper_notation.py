...
from utils.experiments.theory.paper_registry import GROUPS, GROUP_LABELS, GROUP_COLORS, paper_registry, previous_paper_registry
...
def test_all_twenty_axes_use_manuscript_vocabulary_without_changing_existing_contracts():
    ...
    for entry in entries:
        ...
        if entry["stem"] in {
            ...
            expected_y = {
                ...
                "reference_branch_gap_per_prompt": r"$\|\bar{\mathbf{x}}_t(c)-\bar{\mathbf{x}}_t(\varnothing)\|/\sqrt{d}$",
            }
    ...
    assert all(term in margin["x"] for term in (r"\boldsymbol{\Delta}_t", "-e_t(c)", r"-e_t(\varnothing)", "-V_t"))
...
def test_sync_legends_identify_samplewise_maximum_and_complete_bound(tmp_path):
    data = inputs(tmp_path)
    fig, _ = draw(data, "synchronization_bound")
    try:
        text = _legend_texts(fig.axes[0])
        assert r"$\|\boldsymbol{\Delta}_t\|$" in text
        assert r"$e_t(c)+e_t(\varnothing)+R(1-p_t)$" in text
        assert r"$\max_{b\in\{c,\varnothing\}}\|\hat{\mathbf{x}}_t(b)-\mathbf{x}^{\star}\|$" in text
        assert fig.axes[0].get_xlabel() == r"$T-t$"
        assert fig.axes[0].get_ylabel() == r"Magnitude / $\sqrt{d}$"
    finally:
        plt.close(fig)
...
def test_caption_defines_manuscript_symbols_and_preserves_empirical_scope(tmp_path):
    ...
    for stem in ("initial_loss_recovery", "branch_gap_posterior_response", "unconditional_reference_convergence",
        ...
        if stem == "reference_variation_per_prompt":
            assert r"$V_t/\sqrt{d}$" in caption
            ...
            assert "Original saved numerical estimates" in caption
