...
def test_missing_prerequisites_report_seed_aware_commands(
    ...
    with pytest.raises(ProximityError) as missing_generation:
        proximity_module.run_proximity(tmp_path, **arguments)
        ...
        [
            {
                ...
                "selection_status": "included" if index == "1" else "discarded",
                "selection_reason": (
                    "prompt_spearman_lt_0" if index == "1" else "prompt_spearman_ge_0"
                ),
            }
            for index in indices
        ]
    )
    return SimpleNamespace(prompt_frame=prompts)
...
def test_frozen_reference_fast_path_does_not_touch_tensors(
    ...
    directory.mkdir()
    summary = {"complete": True, "selection_hash": "a" * 64}
    (directory / "summary.json").write_text(json.dumps(summary), encoding="utf-8")
    monkeypatch.setattr(
        selection_module,
        "target_pair_selection_directory",
        lambda _root, **_identity: directory,
    )
    monkeypatch.setattr(
        selection_module,
        "load_target_pair_selection",
        lambda _root, **_identity: SimpleNamespace(sha256="a" * 64),
    )
    plotted: list[Path] = []
...
def test_analysis_configuration_is_compact_and_has_no_version_layer(
    tmp_path: Path,
) -> None:
    paths = ProximityPaths.build(
        tmp_path.resolve(),
    ...
    selection = SimpleNamespace(
        configuration={"selection_policy": "prompt_spearman_l2_sscd_lt_zero"},
        sha256="c" * 64,
    )
    config = proximity_module._analysis_configuration(
        paths, RUN_NAME, generation, {"configuration_hash": "b" * 64}, selection
    )
...
def test_configuration_never_accepts_a_different_contract(tmp_path: Path) -> None:
    ...
    proximity_module._write_configuration(path, {"selection_hash": "a" * 64})
    proximity_module._write_configuration(path, {"selection_hash": "a" * 64})
    with pytest.raises(ProximityError, match="incompatible"):
        proximity_module._write_configuration(path, {"selection_hash": "b" * 64})


def test_selection_figure_reloads_frozen_csv_and_ignores_unselected_invalid_rows(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    selection = pd.DataFrame(
        {
            ...
            "prompt": ["kept", "kept", "dropped", "dropped", "bad", "bad"],
            "kind": ["MV", "MV", "TV", "TV", "N", "N"],
            "seed": [3, 4, 3, 4, 3, 4],
            ...
            "generated_image_tile_index": [0, 1, 0, 1, 0, 1],
            "l2_norm": [1.0, 2.0, 1.0, 2.0, float("nan"), float("nan")],
            "sscd": [0.9, 0.1, 0.1, 0.9, float("nan"), float("nan")],
            "prompt_spearman": [-1.0, -1.0, 1.0, 1.0, float("nan"), float("nan")],
            "include_prompt": [True, True, False, False, False, False],
            "selection_status": [
    ...
    def scatter_spy(
        ...
        return real_scatter_figure(frame, statistics)
    ...
    assert len(plotted_frames) == 1
    plotted = plotted_frames[0]
    assert set(plotted["original_index"]) == {"included"}
    assert plotted[["l2_norm", "sscd"]].notna().all().all()
...
def test_analysis_outputs_publish_prompt_level_summary_from_saved_seed_rows(
    ...
    assert sum(tight_layout_calls) == 1
    figure = figures[0]
    ...
    assert legend.get_title().get_text() == "Kind"
