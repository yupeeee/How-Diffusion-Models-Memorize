...
FEEDBACK_CONDITION_LABEL = r"$\|\boldsymbol{\Delta}_t\|>\mathcal{E}_t+\mathcal{V}_t$"
MARGIN = r"\|\boldsymbol{\Delta}_t\|-\mathcal{E}_t-\mathcal{V}_t"
...
TERMINAL_EXPRESSIONS = {
    ...
    "reference": r"e_1(c)+(g-1)[\mathcal{E}_1+R(1-p_1)]",
}
...
NOTATION_SCOPE = (
    ...
    "The requested projected-gap-error-1 contract uses the signed projected error mathcal{E}_t=u_t^T(Delta_t-bar{Delta}_t), where u_t=Delta_t/||Delta_t|| and bar{Delta}_t=bar{x}_t(c)-bar{x}_t(empty). It is evaluated from the actual learned and reference gap vectors, may be negative, and is never replaced by a norm, absolute value or positive part. The unchanged revised mathcal{V}_t takes its positive part only after integrating the signed unit-gap projection of next-step posterior reference minus the current reference. At exactly zero Delta, u=0, mathcal{E}_t=0 and mathcal{V}=0 form an explicit computational extension; no unit direction is claimed. The identity ||Delta||=mathcal{E}_t+u^T bar{Delta}, followed by Cauchy and posterior concentration, gives D<=S and the qualified terminal bound. The condition retains its matched-endpoint qualifications. These are requested derived refinements, not unchanged PDF expressions. Q remains descriptive and is not asserted below S. "
