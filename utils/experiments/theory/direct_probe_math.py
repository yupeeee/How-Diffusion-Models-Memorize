...
def conditional_forward_metrics(target, noise, samples, prediction, alpha, sigma):
    ...
    target = torch.as_tensor(target, dtype=torch.float64, device="cpu").reshape(1, -1)
...
def gaussian_reference_metrics(samples, prediction, reference_mean, mean, alpha, sigma, max_atom_norm):
    """Lemma-2 pointwise decomposition and its fixed finite-law proof bound."""
    a, s, _ = _scales(alpha, sigma)
    z, predicted, posterior = map(_flat, (samples, prediction, reference_mean))
    mean = torch.as_tensor(mean, dtype=torch.float64, device="cpu").reshape(1, -1)
    if z.shape != predicted.shape or z.shape != posterior.shape or mean.shape[1] != z.shape[1]:
...
def pinsker_quantities(target, alpha, sigma):
    ...
    a, s, _ = _scales(alpha, sigma)
    target = torch.as_tensor(target, dtype=torch.float64, device="cpu")
    variance = s * s
    delta = variance - 1
    if abs(delta) < 1e-4:
